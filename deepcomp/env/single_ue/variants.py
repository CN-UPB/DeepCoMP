...
class RelNormEnv(BinaryMobileEnv):
    ...
    * Later: More observations indicating which BS are idle vs used by other UEs; also diff reward
    ...
    def get_ue_obs(self, ue):
        ...
        # TODO test: add avg QoE per BS to obs; normalize to [-1,1]
        avg_util_at_bs = [bs.avg_utility / 20 for bs in self.bs_list]

        # return {'connected': bs_conn, 'dr': bs_norm_dr, 'utility': utility}
        return {'connected': bs_conn, 'dr': bs_norm_dr, 'utility': utility, 'ues_at_bs': ues_at_bs, 'util_at_bs': avg_util_at_bs}
